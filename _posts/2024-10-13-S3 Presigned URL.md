...
4. 저장 공간 절약 및 스케일링 용이

	서버에 파일을 저장하지 않음으로써, 서버에 추가적인 저장 공간이 필요하지 않다. 파일은 클라우드 스토리지에 저장된다.

5. 비용 절감

	서버의 스토리지나 네트워크 사용량이 줄어들면, 서버 운영 비용이 줄어든다. 클라우드 스토리지(S3)는 사용한 만큼 비용을 지불하는 방식이기 때문에, 서버에서 자체적으로 파일을 관리할 때보다 비용 효율적이다.
